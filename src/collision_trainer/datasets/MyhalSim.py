...
class MyhalSimSlam:
    ...
    def __init__(self, only_day_1=False, first_day='', last_day='', day_list=None, map_day=None, verbose=1):
        ...
        if day_list is not None:
            ...
            self.days = np.sort([d for d in self.days if first_day <= d <= last_day])
    ...
    def get_frames_names(self, verbose=1):
        ...
        for d, day in enumerate(self.days):
            ...
            if not exists(frames_folder):
                frames_folder = join(self.days_folder, day, 'classified_frames')
        ...
        # Handle map day
        frames_folder = join(self.days_folder, self.map_day, self.frame_folder_name)
        f_names = [f for f in listdir(frames_folder) if f[-4:] == '.ply']
        f_times = np.array([float(f[:-4]) for f in f_names], dtype=np.float64)
        f_names = np.array([join(frames_folder, f) for f in f_names])
        ordering = np.argsort(f_times)
    ...
    def load_gt_poses_old(self):
        ...
        for d, day in enumerate(self.days):
            ...
            if exists(gt_pkl_file):
                ...
                # Load gt
                day_gt_t = np.loadtxt(gt_csv, delimiter=',', usecols=0, skiprows=1, dtype=np.uint64)
                gt_T = np.loadtxt(gt_csv, delimiter=',', usecols=(5, 6, 7), skiprows=1, dtype=np.float32)
                gt_Q = np.loadtxt(gt_csv, delimiter=',', usecols=(8, 9, 10, 11), skiprows=1, dtype=np.float32)
                ...
                t2 = time.time()
                print('{:s} {:d}/{:d} Done in {:.1f}s'.format(day, d, len(self.days), t2 - t1))
    ...
    def load_loc_poses(self):
        ...
        for d, day in enumerate(self.days):
            ...
            # Out files
            loc_folder = join(self.data_path, 'slam_online', day)
            if not exists(loc_folder):
                makedirs(loc_folder)
    ...
    def load_map_poses(self, day):
        ...
        map_ply_file = join(self.days_folder, day, 'logs-' + day, 'map_traj.ply')
    ...
    def gt_mapping(self, map_voxel_size=0.08, save_group=50, verbose=1):
        ...
        for d, day in enumerate(self.days):
            ...
            for i, f_t in enumerate(frame_times):
                ...
                world_points = np.matmul(world_points, H_velo_world.T).astype(np.float32)[:, :3]
                world_normals = np.matmul(normals, H_velo_world[:3, :3].T).astype(np.float32)
                ...
                if i % save_group == 0:
                    filename = join(out_day_folder, 'map', 'gt_map_{:03d}.ply'.format(i))
                    write_ply(filename,
                ...
                if (i > last_saved_frames + save_group + 1):
                    ...
                    for save_i, save_f_t in enumerate(frame_times[i0:i1]):
                        ...
                        # Load points
                        points, labels = self.load_frame_points_labels(frame_names[i0 + save_i])
                        ...
                        world_pts = np.matmul(world_pts, H_velo_world.T).astype(np.float32)
                        ...
                        traj_pts = np.hstack((traj_pts, np.ones_like(traj_pts[:, :1]) * (i0 + save_i)))
                        all_traj_pts.append(traj_pts.astype(np.float32))
                        all_traj_clrs.append(traj_clrs)

                        traj_pts, traj_clrs = frame_H_to_points(world_H, size=1.1)
                        traj_pts = np.hstack((traj_pts, np.ones_like(traj_pts[:, :1]) * (i0 + save_i)))
                        all_traj_pts2.append(traj_pts.astype(np.float32))
                        all_traj_clrs2.append(traj_clrs)

                    last_saved_frames += save_group
                    filename = join(out_day_folder, 'frames', 'gt_aligned_{:05d}.ply'.format(i0))
                    write_ply(filename,
                              [np.vstack(all_points), np.hstack(all_labels)],
                              ['x', 'y', 'z', 't', 'category'])
                    filename = join(out_day_folder, 'trajectory', 'gt_traj_{:05d}.ply'.format(i0))
                    write_ply(filename,
                              [np.vstack(all_traj_pts), np.vstack(all_traj_clrs)],
                              ['x', 'y', 'z', 't', 'red', 'green', 'blue'])

                    filename = join(out_day_folder, 'trajectory', 'gt_traj_body_{:05d}.ply'.format(i0))
                    write_ply(filename,
                              [np.vstack(all_traj_pts2), np.vstack(all_traj_clrs2)],
                              ['x', 'y', 'z', 't', 'red', 'green', 'blue'])
            ...
            with open(gt_slam_file, 'wb') as file:
                pickle.dump((frame_names[:len(transform_list)],
                             transform_list,
                             1,
                             pointmap), file)
    ...
    def loc_mapping(self, map_voxel_size=0.03, save_group=50, verbose=1):
        ...
        for d, day in enumerate(self.days):
            ...
            online_slam_file = join(out_day_folder, 'loc_map_{:s}.pkl'.format(day))
            ...
            for i, f_t in enumerate(frame_times):
                ...
                # Interpolate the ground truth pose at current time
                interp_t = (f_t - loc_t[loc_i0]) / (loc_t[loc_i1] - loc_t[loc_i0])
                ...
                if i % save_group == 0:
                    filename = join(out_day_folder, 'map', 'loc_map_{:03d}.ply'.format(i))
                    write_ply(filename,
                              [pointmap.points, pointmap.normals, pointmap.scores, pointmap.counts],
                              ['x', 'y', 'z', 'nx', 'ny', 'nz', 'scores', 'counts'])
                ...
                if (i > last_saved_frames + save_group + 1):
                    ...
                    for save_i, save_f_t in enumerate(frame_times[i0:i1]):
                        ...
                        # Interpolate the ground truth pose at current time
                        interp_t = (save_f_t - loc_t[loc_i0]) / (loc_t[loc_i1] - loc_t[loc_i0])
                    ...
                    last_saved_frames += save_group
                    filename = join(out_day_folder, 'frames', 'loc_aligned_{:05d}.ply'.format(i0))
                    write_ply(filename,
                              [np.vstack(all_points), np.hstack(all_labels)],
                              ['x', 'y', 'z', 't', 'category'])
                    filename = join(out_day_folder, 'trajectory', 'loc_traj_{:05d}.ply'.format(i0))
                    write_ply(filename,
                              [np.vstack(all_traj_pts), np.vstack(all_traj_clrs)],
                              ['x', 'y', 'z', 't', 'red', 'green', 'blue'])

                    filename = join(out_day_folder, 'trajectory', 'loc_traj_body_{:05d}.ply'.format(i0))
                    write_ply(filename,
                              [np.vstack(all_traj_pts2), np.vstack(all_traj_clrs2)],
                              ['x', 'y', 'z', 't', 'red', 'green', 'blue'])
    ...
    def old_pointmap_slam(self, map_voxel_size=0.08,
                      frame_stride=1,
        ...
        for d, day in enumerate(self.days):
            ...
            mapping_file = join(out_day_folder, 'PointSLAM_{:s}.pkl'.format(day))
            ...
            for i, f_t in enumerate(frame_times):
                ...
                # Check for failure
                T_error, R_error = rot_trans_diffs(np.stack([gt_H_velo_world, H_velo_world], 0))
                ...
                if (i > last_saved_frames + save_group + 1):
                    ...
                    last_saved_frames += save_group
                    filename = join(out_day_folder, 'frames', 'debug_aligned_{:05d}.ply'.format(i0))
                    write_ply(filename,
                              [np.vstack(all_points)],
                              ['x', 'y', 'z', 't'])
                    filename = join(out_day_folder, 'trajectory', 'debug_traj_{:05d}.ply'.format(i0))
                    write_ply(filename,
                              [np.vstack(all_traj_pts), np.vstack(all_traj_clrs)],
                              ['x', 'y', 'z', 't', 'red', 'green', 'blue'])
    ...
    def pointmap_slam(self, map_voxel_size=0.03,
        ...
        for d, day in enumerate(self.days):
            ...
            if d == 0:
                ...
                # Load map data
                map_file = join(out_folder, 'map_{:s}.ply'.format(self.days[0]))
            ...
            save_trajectory(join(out_folder, 'traj_{:s}.ply'.format(day)), all_H)
            ...
            with open(filename, 'wb') as file:
                pickle.dump((frame_names, all_H[:len(frame_names)], 1, pointmap), file)
    ...
    def map_from_frames(self, frame_names, frames_H,
                        map_voxel_size=0.03,
                        verbose=1):
        ...
        for i, f_H in enumerate(frames_H):
            ...
            # Apply transf
            world_points = np.hstack((points, np.ones_like(points[:, :1])))
            world_points = np.matmul(world_points, f_H.T).astype(np.float32)[:, :3]
            world_normals = np.matmul(normals, f_H[:3, :3].T).astype(np.float32)
    ...
    def refine_map(self, map_dl=0.03, min_rays=10, occup_threshold=0.9):
        ...
        for d, day in enumerate(self.days):
            ...
            movable_prob, movable_count = ray_casting_annot(frame_names, map_points, map_normals, map_H,
        ...
        print('Get ground')
        ground_mask = extract_ground(map_points, map_normals,
                                     map_folder,
        ...
        write_ply(filename,
                  [map_points[mask], map_normals[mask], map_scores[mask], map_counts[mask]],
                  ['x', 'y', 'z', 'nx', 'ny', 'nz', 'scores', 'counts'])
        print('OK')
    ...
    def init_map(self, map_folder, map_dl=0.03):
        ...
        if exists(first_annot_name):
            ...
            # Get the map
            map_original_name = join(self.data_path,
                                     'simulated_runs',
                                     self.map_day,
                                     'logs-' + self.map_day,
                                     'pointmap_00000.ply')
            ...
            # Get short term movables
            movable_prob, movable_count = ray_casting_annot(frame_names, pointmap.points,
                                                            pointmap.normals, map_H,
                                                            theta_dl=1.29 * np.pi / 180,
                                                            phi_dl=0.1 * np.pi / 180,
                                                            map_dl=pointmap.dl,
                                                            motion_distortion=False)
        ...
        for i, f_name in enumerate(frame_names):
            ...
            write_ply(new_f_name,
                      [points, categories],
                      ['x', 'y', 'z', 'cat'])

            t += [time.time()]
            dt = 1000 * (t[-1] - t[0])
            print('Reproj {:s} {:3d}  --- {:5.1f}%   {:6.1f} ms'.format(self.map_day,
                                                                        i + 1,
                                                                        100 * (i + 1) / N,
                                                                        dt))
        print('OK')
        ...
        save_trajectory(join(map_folder, 'init_traj_{:s}.ply'.format(self.map_day)), map_H)
        ...
        save_trajectory(join(map_folder, 'gt_traj_{:s}.ply'.format(self.map_day)), gt_H_velo_world)
        ...
        points, frame_labels = self.load_frame_points_labels(new_frame_names[f_i0])
        normals, planarity, linearity = polar_normals(points, radius=1.5, lidar_n_lines=31, h_scale=0.5, r_scale=1000.0)
        norm_scores = planarity + linearity
        points = points[norm_scores > 0.1]
        normals = normals[norm_scores > 0.1]
        world_points = np.hstack((points, np.ones_like(points[:, :1])))
        world_points = np.matmul(world_points, map_H[f_i0].T).astype(np.float32)[:, :3]
        world_normals = np.matmul(normals, map_H[f_i0][:3, :3].T).astype(np.float32)
        ...
        ground_scores = np.ones(ground_points.shape[0], dtype=np.float32) * 0.99
...
class MyhalSimDataset(PointCloudDataset):
    ...
    def __init__(self, config, day_list, set='training', balance_classes=True, load_data=True):
        ...
        if self.set in ['training', 'validation']:
            ...
            for s_ind, (seq, seq_frames) in enumerate(zip(self.sequences, self.frames)):
                ...
                if False and exists(seq_stat_file):
                    ...
                    # Sequence path
                    seq_path = join(self.original_path, 'annotated_frames', seq)
...
class MyhalSimSampler(Sampler):
    ...
    def calib_max_in(self, config, dataloader, untouched_ratio=0.8, verbose=True, force_redo=False):
        ...
        key = '{:s}_{:d}_{:.3f}_{:.3f}'.format(sampler_method,
    ...
    def calibration(self, dataloader, untouched_ratio=0.9, verbose=False, force_redo=False):
        ...
        for layer_ind in range(self.dataset.config.num_layers):
            ...
            key = '{:s}_{:d}_{:d}_{:.3f}_{:.3f}'.format(sampler_method,
                                                        self.dataset.config.n_frames,
                                                        self.dataset.max_in_p,
                                                        dl,
                                                        r)
